...
/**
 * Cria um payload de transferência (Payment).
 * @param destination - Endereço da carteira de destino.
 * @param amount - Quantidade de XRP a ser enviada.
 * @returns {Promise<ConnectResult>} Dados para abrir a XUMM Wallet e assinar a transação.
 * @throws {Error} Se a resposta HTTP não for OK.
 */
export async function transfer(destination: string, amount: string): Promise<ConnectResult> {
  const res = await fetch(ENDPOINT, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
  ...
  if (!res.ok) {
    throw new Error(`transfer failed: HTTP ${res.status}`);
  }
